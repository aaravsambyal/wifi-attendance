...
function compileModel(modelName, schema) {
  class Model {
    ...
    async save() {
      ...
      // Clean and strip populated references back to raw IDs/data to prevent JSON file pollution
      function cleanValue(val) {
        ...
        if (typeof val === 'object') {
          // If this is a populated mongoose mock document, return its raw string ID
          if (val._id && typeof val._id === 'string' && Object.keys(val).length > 2 && (val.username || val.title || val.meetingId)) {
            return val._id;
          }
          const cleaned = {};
      ...
      for (const k of Object.keys(this)) {
        if (typeof this[k] !== 'function') {
          cleanObj[k] = cleanValue(this[k]);
        }
      }

      // Safeguard index lookup to avoid object reference inequality duplicates
      const targetIdStr = cleanObj._id ? cleanObj._id.toString() : '';
      const idx = allData.findIndex(d => d._id && d._id.toString() === targetIdStr);
      if (idx !== -1) {
  ...
  Model.find = function (query) {
    ...
    const filtered = allData.filter(d => {
      for (const key in query) {
        const valD = d[key] != null ? d[key].toString() : '';
        const valQ = query[key] != null ? query[key].toString() : '';
        if (valD !== valQ) return false;
      }
      return true;
    });
  ...
  Model.findById = function (id) {
    ...
    const allData = readData(modelName);
    const found = allData.find(d => d._id && d._id.toString() === id.toString());
    return new Query(found ? new Model(found) : null, modelName);
  };
  ...
  Model.findByIdAndDelete = function (id) {
    if (!id) return new Query(null, modelName);
    const allData = readData(modelName);
    const idx = allData.findIndex(d => d._id && d._id.toString() === id.toString());
    let deleted = null;
    if (idx !== -1) {
